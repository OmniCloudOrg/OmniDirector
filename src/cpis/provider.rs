...
pub enum ParseRules {
    #[serde(rename = "object")]
    Object { patterns: HashMap<String, Pattern> },
    #[serde(rename = "array")]
    Array {
        separator: String,
...
impl Provider {
    ...
    pub fn get_action(&self, action_name: &str) -> Result<&ActionDef, CpiError> {
        trace!(
            "Looking up action '{}' in provider '{}'",
            action_name,
            self.name
        );
        self.actions.get(action_name).ok_or_else(|| {
            error!(
                "Action '{}' not found in provider '{}'",
...
pub fn load_provider(path: PathBuf) -> Result<Provider, CpiError> {
    ...
    info!(
        "Successfully loaded provider '{}' with {} actions in {:?}",
        provider.name,
    ...
    trace!(
        "Provider details: type={}, actions={}, has_default_settings={}",
...
pub fn load_providers_from_dir(dir: &PathBuf) -> Result<Vec<Provider>, CpiError> {
    ...
    let elapsed = start.elapsed();
    info!(
        "Successfully loaded {} providers in {:?}",
        providers.len(),
        elapsed
    );
