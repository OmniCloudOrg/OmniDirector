...
pub fn load_all_extensions(cpi_dir: &Path) -> Result<()> {
    println!("Loading CPIs from directory: {:?}", cpi_dir);

    if !cpi_dir.exists() {
        std::fs::create_dir_all(cpi_dir)
            .map_err(|e| {
    ...
    let entries = std::fs::read_dir(cpi_dir)
        .map_err(|e| {
        ...
        match entry {
            Ok(entry) => {
                ...
                if path.is_file() && has_target_extension(&path, target_extension) {
                    println!("\x1b[34mCPI will load: {:?}\x1b[0m", entry.file_name());
                    ...
                    println!("\x1b[33mCPI will not load: {:?} - {}\x1b[0m",
                        entry.file_name(),
                        if !path.is_file() { "Not a file" } 
    ...
    // Use comfy-table to create a nicely formatted box for the loading results
    let mut table = Table::new();
    table
        .set_header(vec![Cell::new("CPI LOADING SUMMARY").fg(Color::Green).add_attribute(comfy_table::Attribute::Bold)])
        .add_row(Row::from(vec![
            Cell::new("✅ Successfully Loaded:").fg(Color::Green),
            Cell::new(loaded_count.to_string()).fg(Color::Green)
        ]))
        .add_row(Row::from(vec![
            Cell::new("❌ Failed to Load:").fg(Color::Red),
            Cell::new(failed_count.to_string()).fg(Color::Red)
        ]))
        .add_row(Row::from(vec![
            Cell::new("📊 Total Processed:").fg(Color::Blue),
...
pub fn load_extension(path: &Path) -> Result<()> {
    ...
    unsafe {
        let init_fn: libloading::Symbol<unsafe extern "C" fn() -> i32> = library
            .get(b"cpi_init")
            .map_err(|e| {
        ...
        if result != 0 {
            ...
            return Err(CpiError::InitializationFailedError(name, anyhow::Error::new(io_error)));
        }
    }
